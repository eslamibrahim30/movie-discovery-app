    ...
    <>
      <nav className="sticky top-0 z-40 flex items-center justify-between p-4 border-b border-border bg-background/80 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            asChild
            className="hidden sm:flex rounded-full px-6 shadow-md shadow-primary/20"
          >
            <Link to="/account">
      ...
      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-50 transition-all ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}
        onClick={() => setIsOpen(false)}
      />

      {/* Sidebar (Mobile) */}
      <aside
        className={`fixed top-0 ${lang === "EN" ? "right-0" : "left-0"} h-full w-72 bg-background z-50 transition-transform duration-300 shadow-2xl ${isOpen ? "translate-x-0" : lang === "EN" ? "translate-x-full" : "-translate-x-full"}`}
      >
        ...
        <div className="p-4 flex flex-col gap-2">
          ...
          <div className="mt-4 pt-4 border-t sm:hidden">
            ...
            >
              <Link to="/account">
                <User size={20} className="me-2" /> {t.account}
              </Link>
            </Button>
          </div>
        </div>
